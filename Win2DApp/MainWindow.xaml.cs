    ...
    public sealed partial class MainWindow : Window
    {
        ...
        private void AnimatedCanvas_Update(ICanvasAnimatedControl sender, CanvasAnimatedUpdateEventArgs args)
        {
            ...
            for (int i = 0; i < Cube.CopyVertices.Count; ++i)
            {
                ...
                Cube.CopyVertices[i] = ProjectionMatrix.RotateX(v, angle);
                Cube.CopyVertices[i].z += 5;
                Cube.CopyVertices[i] = ProjectionMatrix.Project(Cube.CopyVertices[i], projectionMatrix);
            }
